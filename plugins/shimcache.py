# ShimCacheParser.py
# Based on the great ShimCacheParser by Mandiant
# (Andrew Davis, andrew.davis@mandiant.com
# Copyright 2012 Mandiant)
# Under apache 2 licence
# Available here : https://github.com/mandiant/ShimCacheParser
import sys
import struct
import zipfile
import argparse
...
class CacheEntryNt5(object):
    """Shim Cache format used by Windows 5.2 and 6.0 (Server 2003 through
    Vista/Server 2008)"""
...
class PluginShimcache(Plugin):
    name = "shimcache"
    htype = ["SYSTEM"]
    description = "Extract shimcache entries"
    ...
    def convert_filetime(self, dwLowDateTime, dwHighDateTime):
    ...
    def read_winxp_entries(self, bin_data):
        entry_list = []
        ...
        try:
            ...
            for offset in xrange(self.WINXP_HEADER_SIZE32,
                                (num_entries*self.WINXP_ENTRY_SIZE32), self.WINXP_ENTRY_SIZE32):

                # No size values are included in these entries, so search for utf-16 terminator.
                path_len = bin_data[offset:offset+(self.MAX_PATH + 8)].find("\x00\x00")
                ...
                entry_data = (offset+(self.MAX_PATH+8))

                # Get last mod time.
                last_mod_time = struct.unpack('<2L', bin_data[entry_data:entry_data+8])
                try:
                    last_mod_time = self.convert_filetime(last_mod_time[0],
                ...
                if file_size == 0:
                    file_size = self.bad_entry_data

                # Get last update time.
                exec_time = struct.unpack('<2L', bin_data[entry_data + 16:entry_data + 24])
                try:
                    exec_time = self.convert_filetime(exec_time[0],
                                                exec_time[1]).strftime("%m/%d/%y %H:%M:%S")
                except ValueError:
                    exec_time = self.bad_entry_data
    ...
    def read_nt6_entries(self, bin_data, entry):
        try:
            entry_list = []
            exec_flag = ""
            ...
            for offset in xrange(CACHE_HEADER_SIZE_NT6_1,
                ...
                last_mod_date = self.convert_filetime(entry.dwLowDateTime,
    ...
    def read_nt5_entries(self, bin_data, entry):
        try:
            entry_list = []
            ...
            for offset in xrange(CACHE_HEADER_SIZE_NT5_2, (num_entries  * entry_size),
                ...
                last_mod_date = self.convert_filetime(entry.dwLowDateTime, entry.dwHighDateTime)
                try:
                    last_mod_date = last_mod_date.strftime("%m/%d/%y %H:%M:%S")
                except ValueError:
                    last_mod_date = self.bad_entry_data
    ...
    def read_win8_entries(self, bin_data, ver_magic):
        offset = 0
        entry_meta_len = 12
        entry_list = []

        # Skip past the stats in the header
        cache_data = bin_data[self.WIN8_STATS_SIZE:]
        ...
        while data.tell() < len(cache_data):
            ...
            last_mod_date = self.convert_filetime(low_datetime, high_datetime)
            try:
                last_mod_date = last_mod_date.strftime("%m/%d/%y %H:%M:%S")
            except ValueError:
                last_mod_date = self.bad_entry_data
    ...
    # Reiad the Shim Cache format, return a list of last modified dates/paths.
    def read_cache(self, cachebin, quiet=True):
        if len(cachebin) < 16:
        ...
        try:
            ...
            elif len(cachebin) > self.WIN8_STATS_SIZE and cachebin[self.WIN8_STATS_SIZE:self.WIN8_STATS_SIZE+4] == self.WIN8_MAGIC:
                if not quiet:
                    print "[+] Found Windows 8/2k12 Apphelp Cache data..."
                return self.read_win8_entries(cachebin, self.WIN8_MAGIC)

            # Windows 8.1 will use a different magic dword, check for it
            elif len(cachebin) > self.WIN8_STATS_SIZE and cachebin[self.WIN8_STATS_SIZE:self.WIN8_STATS_SIZE+4] == self.WIN81_MAGIC:
                if not quiet:
                    print "[+] Found Windows 8.1 Apphelp Cache data..."
                return self.read_win8_entries(cachebin, self.WIN81_MAGIC)
    ...
    def run(self, reg, verbose, params={}):
                ...
                if 'controlset' in key.name().lower():
                    ...
                    for subkey in session_man_key.subkeys():
                        ...
                        if ('appcompatibility' in subkey.name().lower() or
                            ...
                            if verbose > 0:
                                tmp_list = self.read_cache(bin_data, quiet=False)
                            else:
                                tmp_list = self.read_cache(bin_data, quiet=True)
        ...
        if len(out_list) == 0:
            ...
            if verbose > 0:
                out_list.insert(0, self.output_header + ['Key Path'])
                return out_list
            else:
            # Only return unique entries.
                out_list = self.unique_list(out_list)
                out_list.insert(0, self.output_header)
                return out_list
